...
void getaddu(Mat, uint8 *);

void fillmatu(uint8 *, Mat);
...
Mat imPadding(Mat I, int pt, int pb, int pl, int pr) {
    uint8 *I_data = new uint8[I.rows * I.cols * I.channels()];
    getaddu(I, I_data);
    uint8 *O_data = new uint8[(I.rows + pt + pb) * (I.cols + pl + pr) * I.channels()];
    for (int i = 0; i < (I.rows + pt + pb) * (I.cols + pl + pr) * I.channels(); i++) O_data[i] = 0;
    imPad<uint8>(I_data, O_data, I.rows, I.cols, I.channels(), pt, pb, pl, pr, 2, 0);
    Mat O(I.rows + pt + pb, I.cols + pl + pr, CV_8UC3);
    fillmatu(O_data, O);
    return O;
};